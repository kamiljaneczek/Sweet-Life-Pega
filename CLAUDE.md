...
`src/index.tsx` renders a React Router v5 (`BrowserRouter`/`Switch`) app with static pages: Home, Company, Products, Contact, Support, DesignSystem. Each page is a standalone component in `src/app/`.
...
**Custom Constellation components** (`src/components/custom-constellation/`) — Similar to custom-sdk but for the Constellation component model.
