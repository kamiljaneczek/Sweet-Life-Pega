...
const Contact = () => {
  ...
  function createInquiryCase() {
    let token = '';
    (async () => {
      const responseToken = await fetch('/prweb/PRRestService/oauth2/v1/token', {
        method: 'POST',
        headers: {
      ...
      const responseCase = await fetch('/prweb/api/application/v2/cases?viewType=none&pageName=', {
        method: 'POST',
        headers: {
      ...
      setCaseID(dataCase.data.caseInfo.businessID);
  ...
  return (
    <>
      <Header />
      {!isPegaReady && <Loading />}
      <div className={classNames('flex-grow bg-white dark:bg-gray-900', { hidden: !isPegaReady })}>
        <section className=''>
