/**
 * Generated by orval v8.5.0 🍺
 * Do not edit manually.
 * PegaAPI
 * The Pega API provides a set of built-in REST services for Pega applications. All APIs require authentication. Provide the credentials for a Pega operator in the Authorization header for each request. Pega recommends accessing APIs using TLS 1.2.
 * OpenAPI spec version: v1
 */
import type { GetMessagesFilterBy } from './getMessagesFilterBy';

export type GetMessagesParams = {
  /**
   * if filtering by context, specify it in filterFor
   */
  filterBy?: GetMessagesFilterBy;
  /**
   * context or user ID
   */
  filterFor?: string;
  /**
   * max entries to return
   */
  pageSize?: number;
  /**
   * Only return entries older than this date time
   */
  olderThan?: string;
};
