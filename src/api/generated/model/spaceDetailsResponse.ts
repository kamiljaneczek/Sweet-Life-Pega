/**
 * Generated by orval v8.5.0 🍺
 * Do not edit manually.
 * PegaAPI
 * The Pega API provides a set of built-in REST services for Pega applications. All APIs require authentication. Provide the credentials for a Pega operator in the Authorization header for each request. Pega recommends accessing APIs using TLS 1.2.
 * OpenAPI spec version: v1
 */
import type { SpacesMembers } from './spacesMembers';
import type { SpacesOwner } from './spacesOwner';
import type { SpacesSpacehierarchy } from './spacesSpacehierarchy';

export interface SpaceDetailsResponse {
  owner?: SpacesOwner;
  createdByName?: string;
  lastUpdatedBy?: string;
  accessType?: string;
  memberCount?: string;
  description?: string;
  spacehierarchy?: SpacesSpacehierarchy[];
  createdBy?: string;
  createTime?: string;
  members?: SpacesMembers[];
  name?: string;
  ID?: string;
  lastUpdatedByName?: string;
  lastUpdateTime?: string;
}
